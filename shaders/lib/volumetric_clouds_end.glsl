/* MakeUp - E-LITE shaders 5 - volumetric_clouds_end.glsl
...
vec3 get_end_cloud(vec3 view_vector, vec3 block_color, float bright, float dither, vec3 base_pos, int samples) {
    ...
    if (view_vector.y > 0.0) {  // Vista sobre el horizonte
        ...
        vec3 dark_cloud_color = block_color * 0.75;
        ...
        for (int i = 0; i < samples; i++) {
            ...
            #else
                float current_value = texture2D(colortex2, (intersection_pos.xz * .0008) + (frameTimeCounter * CLOUD_HI_FACTOR * 3.0)).r;
            #endif
